#!/usr/bin/env python3
"""
Token Usage Analyzer for OpenClaw - FIXED VERSION
Fixed token parsing issue where "39k" was interpreted as 39,000 instead of 39.
"""
...
def parse_session_tokens(status_output):
    """Parse session token usage from status output - FIXED VERSION"""
    sessions = []
    ...
    for line in lines:
        ...
        if in_sessions:
            ...
            if match:
                ...
                print(f"DEBUG: Parsing tokens: '{tokens}'")  # Debug
                ...
                if token_match:
                    ...
                    # FIX: The "k" is part of the number format, NOT a thousands multiplier
                    ...
                    print(f"DEBUG: used_tokens={used_tokens}, limit_tokens={limit_tokens}, percent={percent}")  # Debug
                    ...
                    sessions.append({
                        ...
                        'raw_tokens_string': tokens  # For debugging
                    })
...
def generate_report(sessions, total_cost):
    ...
    report = []
    report.append("=" * 60)
    report.append("📊 OPENCLAW TOKEN USAGE REPORT (FIXED)")
    report.append("=" * 60)
    ...
    for session in sessions:
        ...
        report.append(f"   Raw string: {session.get('raw_tokens_string', 'N/A')}")
...
def main():
    print("🔍 Analyzing OpenClaw token usage (FIXED VERSION)...")
